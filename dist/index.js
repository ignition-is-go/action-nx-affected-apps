...
function getNxAffectedApps({ base, head, workspace }) {
    ...
    let result;
    try {
        result = child_process_1.execSync(`nx affected:apps ${args}`, {
            cwd: workspace
        }).toString();
    }
    catch (_a) {
        try {
            result = child_process_1.execSync(`./node_modules/.bin/nx affected:apps ${args}`, {
                cwd: workspace
            }).toString();
        }
        catch (_b) {
            result = child_process_1.execSync(`npm run nx -- affected:apps ${args}`, {
                cwd: workspace
            }).toString();
        }
    }
